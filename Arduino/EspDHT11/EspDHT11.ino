...
void setup() {
	...
	esp_now_add_peer(macPasarela, 3, WIFI_CHANNEL, key, 0);
	//Especificamos las claves para enviar la información de manera encriptada.
	...
	esp_now_register_recv_cb([](uint8_t *mac, uint8_t *data, uint8_t len) {
		...
		//Guardamos la acción a realizar.
		char action = (char) data[36];

		//Si es cambiar el led...
		if(action == 'l'){

		  //Obtenemos el valor nuevo del LED.
		  char led = (char) data[42];
		  
		  //Si llega un 1, encendemos el LED, si llega un 0, apagamos el LED.
		  if(led == '1'){ espNowLEDValue = 1; }
		  if(led == '0'){ espNowLEDValue = 0; }
		  
		  if(espNowLEDValue != currentLEDValue){
			currentLEDValue = espNowLEDValue;
		  
			if(currentLEDValue == 1){
		...
		//Si es cambiar la frecuencia...
		if(action == 'f'){

		  //Obtenemos el valor de la frecuencia.
		  int frecuencia1 = ((char) data[48] - 48)*10;
		  int frecuencia2 = ((char) data[49] - 48);
...
void loop() {
  ...
  if(countTime<millis()){
    ...
    sprintf(mensaje,"{\"hum\":%g,\"temp\":%g,\"status\":\"%s\"}", h, t, dht.getStatusString());
    ...
    countTime = millis() + frecuenciaActualizacion*1000;
  	...
  	//Si da la casualidad de que, después de enviar los datos del sensor, también va a enviar el 
	//mensaje de que está vivo, lo retrasamos un segundo para que no haya interferencias.
